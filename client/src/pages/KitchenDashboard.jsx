...
export default function PremiumKitchenDashboard() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const fifoOrdered = res.data.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
      setOrders(fifoOrdered);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    socket.on('order-created', () => { if (activeTab === 'TO_COOK') fetchOrders(); });
    socket.on('order-status-updated', () => fetchOrders());
    socket.on('item-status-toggled', () => fetchOrders());

    return () => { socket.disconnect(); };
  }, [activeTab]);

  // 1. 📊 Top Row Macro-Aggregation Engine
  // Calculates total quantities of individual pending products across all displayed orders
  ...
  // Extract unique items and categories for the dynamic sidebar filter menu
  const uniqueProductsForFilter = useMemo(() => {
    const products = new Set();
    orders.forEach(o => o.order_items?.forEach(i => products.add(i.product?.name)));
  ...
  const handleToggleItemStrike = async (order, itemId, currentCompleted) => {
    try {
      ...
      if (order.status === 'PREPARING' && !currentCompleted) {
        const remainingItems = order.order_items.filter(i => i.id !== itemId && !i.completed);
  ...
  // 2. 🔍 Multi-Tiered Filtering Logic
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      // Sidebar item name filters
      const matchesProduct = selectedProductFilter === 'ALL' || order.order_items?.some(item => item.product?.name === selectedProductFilter);
      
      // Sidebar category mock matches (Member 2 schema maps product category_id)
      const matchesCategory = selectedCategoryFilter === 'ALL' || order.order_items?.some(item => item.product?.category_id === parseInt(selectedCategoryFilter));

      return matchesSearch && matchesProduct && matchesCategory;
    });
  }, [orders, searchQuery, selectedProductFilter, selectedCategoryFilter]);

  return (
    <div className="min-h-screen bg-[#FDFBF7] text-stone-800 font-sans flex flex-col">
      
      {/* Upper Navigation & Status Top Bar */}
      <header className="p-4 md:p-6 border-b border-stone-200/60 bg-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-stone-500 text-xs">Real-time Live FIFO Stream Dashboard</p>
        </div>
      ...
      {/* 🚀 Top Row Cumulative Product Display Banner */}
      <div className="bg-white border-b border-stone-200 px-6 py-3 min-h-[52px] flex items-center overflow-x-auto gap-3">
        <span className="text-xs uppercase font-extrabold tracking-wider text-stone-400 mr-2 whitespace-nowrap">Prep Summary:</span>
        {macroSummary.length === 0 ? (
          ...
          <div className="flex gap-2">
            {macroSummary.map((item, idx) => (
              <div key={idx} className="bg-stone-900 text-[#FDFBF7] px-3 py-1 rounded-lg flex items-center gap-2 text-xs font-semibold shadow-sm">
                <span className="text-amber-400 font-mono">{item.qty}x</span>
                <span>{item.name}</span>
              </div>
            ))}
          </div>
      ...
      {/* Main Body Grid Dashboard Area with Sidebar Filter */}
      <div className="flex flex-1 flex-col lg:flex-row">
        
        {/* 📋 Left Sidebar Filter Dashboard Control Panel */}
        <aside className="w-full lg:w-64 bg-white border-r border-stone-200 p-4 flex flex-col gap-6">
          ...
          {/* Dynamic Active Tab Counter Toggles inside Sidebar */}
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {['TO_COOK', 'PREPARING', 'COMPLETED'].map((status) => (
                ...
                >
                  ...
                  <span className="font-mono text-[10px] bg-stone-200 text-stone-800 px-1.5 py-0.5 rounded">{activeTab === status ? filteredOrders.length : '•'}</span>
                </button>
              ))}
            </div>
          </div>

          {/* Product Items Filter Links */}
          <div>
          ...
          {/* Category ID Filters (Mapping Member 2 structure tags) */}
          <div>
        ...
        {/* 🗂️ Main Kitchen Ticket Column Display Canvas Grid */}
                    ...
                    <div className={`p-4 border-b border-stone-100 flex justify-between items-start ${
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] uppercase tracking-wider font-extrabold text-stone-400">Table {order.table_id}</span>
                          {index === 0 && (
                      ...
                      <span className="text-xs font-semibold bg-stone-100 border border-stone-200 px-2 rounded-md text-stone-700">{order.customer_name}</span>
                    </div>
                    ...
                    <div className="p-4 divide-y divide-stone-100 max-h-[240px] overflow-y-auto">
                      {order.order_items?.map((item) => (
                        <div 
                          ...
                          className={`py-2.5 flex items-center justify-between select-none ${order.status === 'PREPARING' ? 'cursor-pointer group/line' : 'pointer-events-none'}`}
                        >
                          <div className="flex items-center space-x-3 min-w-0">
                            <span className={`font-mono text-xs font-bold px-1.5 py-0.5 rounded ${item.completed ? 'bg-stone-100 text-stone-400' : 'bg-stone-900 text-[#FDFBF7]'}`}>{item.quantity}x</span>
                            <span className={`text-xs font-medium truncate ${item.completed ? 'line-through text-stone-400 italic' : 'text-stone-800'}`}>{item.product?.name || "Café Item"}</span>
                          </div>
